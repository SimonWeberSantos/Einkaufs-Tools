...
function toIdPart(name) {
  return name.replace(/\s+/g, "_");
}

// Sicherheitszuschlag: einzige Stelle für den 10%-Puffer
const PUFFER = 0.1;
...
function getMaterialOrder() {
  ...
  rowMeta.forEach((meta) => {
    if (!seen.has(meta.materialName)) {
      seen.add(meta.materialName);
      order.push(meta.materialName);
    }
  });
  return order;
}
...
function buildRows(data) {
  ...
  seenMaterials.forEach((name) => {
    ...
    tr.innerHTML =
      ...
      `<td class="cell-right cell-buffer" id="total-buf-${toIdPart(name)}">0.00</td>`;
...
function renderDesktop() {
  ...
  // Material-Totale aktualisieren (Basis + inkl. Puffer)
  getMaterialOrder().forEach((name) => {
    const basis = totals[name] || 0;
    const el = document.getElementById(`total-${toIdPart(name)}`);
    if (el) el.textContent = basis.toFixed(2);
    const bufEl = document.getElementById(`total-buf-${toIdPart(name)}`);
    if (bufEl) bufEl.textContent = (basis * (1 + PUFFER)).toFixed(2);
  });
}
...
function buildMobileSelects(data) {
  ...
  productSel.addEventListener("change", () => {
    ...
    repopulateMaterials(
      pIdx,
      parseInt(document.getElementById("m-select-size").value, 10),
    );
    cancelEditMode();
  });
...
function findRid(pIdx, size, materialName) {
  for (const [rid, meta] of rowMeta) {
    if (
      meta.pIdx === pIdx &&
      meta.size === size &&
      meta.materialName === materialName
    )
      return rid;
  }
  return null;
...
function renderMobileEntries() {
  ...
  const active = [];
  rowMeta.forEach((meta, rid) => {
    if (getQty(rid) > 0) active.push({ rid, ...meta });
  });
  ...
  list.innerHTML = active
    .map(({ rid, productName, size, materialName, weight }) => {
      const qty = getQty(rid);
...
function renderMobileTotals() {
  ...
  container.innerHTML = getMaterialOrder()
    ...
    .map((name) => {
      const basis = totals[name];
      const bestell = (basis * (1 + PUFFER)).toFixed(2);
      return (
        '<div class="m-totals__row">' +
        `<span class="m-totals__label">${name}</span>` +
        '<span class="m-totals__values">' +
        `<span class="m-totals__basis">${basis.toFixed(2)}\u202fkg</span>` +
        `<span class="m-totals__buffer">+\u202f${Math.round(PUFFER * 100)}\u202f%\u202f=\u202f${bestell}\u202fkg</span>` +
        "</span>" +
        "</div>"
      );
    })
...
function setupMobileEvents() {
  ...
  document.getElementById("m-btn-confirm").addEventListener("click", () => {
    ...
    if (editRid >= 0) {
      ...
      const pIdx = parseInt(
        document.getElementById("m-select-product").value,
        10,
      );
...
function buildCopyContent() {
  ...
  const totals = computeTotals();
  const activeMaterials = getMaterialOrder().filter(
    (n) => (totals[n] || 0) > 0,
  );

  // ── HTML-Tabelle (inline Styles für Word/Outlook) ──────────────
  const tdStyle =
    "padding:4px 8px;border:1px solid #cfd8dc;font-family:Calibri,Arial,sans-serif;font-size:11pt;";
  const thStyle =
    tdStyle + "background:#d4f0d6;color:#1b5e20;font-weight:bold;";

  const htmlDataRows = activeRows
    .map((r) => {
  ...
  const htmlTotalRows = activeMaterials
    .map((name) => {
      const basis = totals[name];
      const bestell = (basis * (1 + PUFFER)).toFixed(2);
      return (
        `<tr>` +
        `<td colspan="4" style="${tdStyle}color:#757575;">${name}</td>` +
        `<td style="${tdStyle};text-align:right;">${basis.toFixed(2)} kg</td>` +
        `<td style="${tdStyle};text-align:right;font-weight:bold;color:#1b5e20;">${bestell} kg</td>` +
        `</tr>`
      );
    })
    .join("");
  ...
  const html =
    ...
    `<tbody>` +
    `<tr><td colspan="4" style="${thStyle}">Gesamtbedarf pro Material</td><td style="${thStyle};text-align:right;">Basis (kg)</td><td style="${thStyle};text-align:right;">Granulat inkl.\u202f10\u202f% (kg)</td></tr>` +
    `${htmlTotalRows}` +
    `</tbody>` +
    `</table>`;
  ...
  const plain = [
    ...
    ...activeRows.map((r) =>
      [
        ...
        r.qty,
        r.weight.toFixed(2),
        (r.qty * r.weight).toFixed(2),
      ].join("\t"),
    ),
    "",
    "Gesamtbedarf pro Material",
    ...activeMaterials.map(
      (name) => `${name}\t\t\t\t\t${totals[name].toFixed(2)} kg`,
    ),
  ].join("\n");
...
async function init() {
  ...
  try {
    ...
    document
      .getElementById("copy-btn-desktop")
      .addEventListener("click", function () {
        copyResults(this);
      });
    document
      .getElementById("copy-btn-mobile")
      .addEventListener("click", function () {
        copyResults(this);
      });
    ...
    showError(
      "Stammdaten konnten nicht geladen werden. Bitte Seite neu laden.",
    );
  } finally {
    setLoading(false);
  }
